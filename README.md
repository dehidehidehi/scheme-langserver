...
>NOTE
Scheme-langserver's multi-threaded feature requires [Chez Scheme](https://cisco.github.io/ChezScheme/) to have been compiled with the `--threads` option and the `--kernelobj` option. For Chez Scheme version after 10.0.0, there're some differents now.
